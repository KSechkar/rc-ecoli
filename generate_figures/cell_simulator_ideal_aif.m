%% cell_simulator_ideal_aif.m
% Matlab class enabling simulations of the host cell - but for the AIF
% controller the actuator and the annihilator RNA's degradadation and
% dilution terms omitted
...
classdef cell_simulator_ideal_aif
    ...
    methods (Access = public)
        % CONSTRUCTOR
        function obj = cell_simulator_ideal_aif(tf)
        ...
        function dxdt = ss_model(obj, t, x)
            ...
            psi=obj.form.psi(par,T); % tRNA synthesis rate
            ...
            dxdt = [
                    ...
                    obj.form.F_r(par,T).*par('c_r').*par('a_r')-(par('b_r')+l).*m_r;
                    % ,metabolic protein a
            ...
            if(obj.num_het>0)
                ...
                for i=1:obj.num_het
                    % mRNA
                    if(strcmp(obj.het.names{i},'anti') || strcmp(obj.het.names{i},'act')) % NO DEGRADATION\DILUTION for actuator and annihilator
                    ...
                    else % for other mRNAs, everything is as usual
                        dxdt_het(i)=obj.het.regulation(obj.het.names{i},x,ext_inp)...
                            .*par(['c_',obj.het.names{i}]).*par(['a_',obj.het.names{i}])...
