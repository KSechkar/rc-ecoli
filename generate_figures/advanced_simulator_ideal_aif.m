...
classdef advanced_simulator_ideal_aif
    ...
    methods (Access = public)
        ...
        function obj = advanced_simulator_ideal_aif(tf) % Constructor
            if nargin == 1
                obj.tf = tf;
            end
        ...
        function dxdt = ss_model(obj, t, x)
            ...
            if(obj.num_het>0)
                ...
                % calculate
                for i=1:obj.num_het
                    if(strcmp(obj.het.names{i},'act') || strcmp(obj.het.names{i},'anti'))
                        dxdt_het(i)=obj.het.regulation(obj.het.names{i},x,ext_inp).*...
                            ...
                            %+par(['b_',obj.het.names{i}]).*x_het(i);
                    else
                        dxdt_het(i)=obj.het.regulation(obj.het.names{i},x,ext_inp)...
                            .*par(['c_',obj.het.names{i}]).*par(['a_',obj.het.names{i}])...
                            -(par(['b_',obj.het.names{i}])+l).*x_het(i)...
                            +obj.het.extra_m_term(obj.het.names{i},x,ext_inp)...
                            -kcmh.*(x_het(i)./k_het(i)./D).*R; % MOD!!!
                    end
