% integral feedback, but now disturbance linearly depends on external input belonging to [0;1]

classdef integral_dirdist
    ...
    methods (Access = public)
        function obj = integral_dirdist(obj)
            obj.module_name='integral_dirdist';
            ...
            obj.misc_names={'bound'}; % bound actuator and annihilator
            ...
            obj.parameters('kb_anti')=10; % (what would a realistic value be?)
            obj.parameters('b_bound')=6; % antisense degradation rate

            % m_anti NOT transcribed
            obj.parameters('k+_anti')=0;
        ...
        function F = regulation(obj,gene_name,x,ext_inp)
            ...
            elseif (strcmp(gene_name,'dist'))
                F=ext_inp(1); % inducer concentration (AHL)             
        ...
        function extra_term=extra_m_term(obj,gene_name,x,ext_inp)
            ...
            % extra term for m_act-m_anti binding
        ...
        function dxdt=misc_ode(obj,t,x,ext_inp,l)
            ...
            dxdt=het_par('kb_anti').*m_anti.*m_act - (het_par('b_bound')+l).*anti;
        end
    end
end